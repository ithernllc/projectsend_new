...
function getUsersWithTrialEndingTomorrow()
{
    try {
        ...
        $sql = "
             SELECT id, user, email, trial_start_date 
        FROM users 
        WHERE subscription_status = 'trial' 
        AND DATE_ADD(trial_start_date, INTERVAL 30 DAY) = :tomorrow 
        ";
        global $dbh;
        // Prepare and execute the statement
        $statement = $dbh->prepare($sql);
        $statement->execute([':tomorrow' => $tomorrow]);
        $statement->setFetchMode(PDO::FETCH_ASSOC);
...
function getUsersWithPaymentDueToday()
{
    try {
        $today = date('Y-m-d');

        // SQL Query to fetch users whose payment is due today
        $sql = "
            SELECT id, name, email, subscription_start_date,plan ,cvv,card_number,expiry_date
            FROM users WHERE subscription_start_date = :today
        ";
...
function markSubscriptionAsPaid($userId)
{
    try {
        // SQL Query to update the subscription_status
        $sql = "UPDATE users SET subscription_status = 'active' WHERE id = :userId";

        global $dbh; // Assuming $dbh is your PDO instance
        $statement = $dbh->prepare($sql);

        // Execute the query
        $statement->execute([':userId' => $userId]);

        echo "Subscription status updated to active for user ID {$userId}.\n";
    } catch (PDOException $e) {
        // Handle database errors
        echo "Database error while updating subscription status: " . $e->getMessage();
    }
}
...
{
    try {
        // SQL Query to update the subscription_status to 'inactive'
        $sql = "UPDATE users SET subscription_status = 'inactive' WHERE id = :userId";
        ...
        echo "Subscription status updated to inactive for user ID {$userId}.\n";
    } catch (PDOException $e) {
        // Handle database errors
        echo "Database error while updating subscription status: " . $e->getMessage();
    }
