...
class Users
{
    ...
    function cancelSub() {
        ...
        require __DIR__ . '/../../stripe-php/vendor/autoload.php'; // Adjusted path
        ...
        global $dbh; // Use your database connection

        try {
            
            $fetch_stripe_subscription_id = "SELECT stripe_subscription_id from users  WHERE id = :user_id";
            $statement_fetch_stripe_subscription_id = $dbh->prepare($fetch_stripe_subscription_id);
            $statement_fetch_stripe_subscription_id->execute([':user_id' => $this->id]);
            $statement_fetch_stripe_subscription_id->setFetchMode(PDO::FETCH_ASSOC);
            $row = $statement_fetch_stripe_subscription_id->fetch();

            if (empty($row['stripe_subscription_id'])) {
                return "error: No subscription ID found.";
            }
            // **Cancel the subscription at period end**
            $subscription = \Stripe\Subscription::update($row['stripe_subscription_id'], [
                'cancel_at_period_end' => true, // Allow access until the billing period ends
            ]);
    
            $query = "UPDATE users SET subscription_status = 'pending_cancellation' WHERE id = :user_id";
            ...
            return "error: " . $e->getMessage(); // Return the Stripe API error message
        }
    ...
    function updatebilling() {
        ...
        try {
            ...
            $fetch_stripe_subscription_id = "SELECT stripe_customer_id from users  WHERE id = :user_id";
            $statement_fetch_stripe_subscription_id = $dbh->prepare($fetch_stripe_subscription_id);
            $statement_fetch_stripe_subscription_id->execute([':user_id' => $this->id]);
            $statement_fetch_stripe_subscription_id->setFetchMode(PDO::FETCH_ASSOC);
            $row = $statement_fetch_stripe_subscription_id->fetch();
    ...
    {
        $this->name = (!empty($arguments['first_name']) && !empty($arguments['last_name'])) ? encode_html($arguments['first_name'] . ' ' . $arguments['last_name']) : null;
        ...
        $this->plan = (!empty($arguments['plan'])) ? $arguments['plan'] : null;
        $this->trial_start_date = (!empty($arguments['trial_start_date'])) ? $arguments['trial_start_date'] : null;
        $this->stripeToken = (!empty($arguments['stripeToken'])) ? $arguments['stripeToken'] : null;
        $this->subscription_start_date = (!empty($arguments['subscription_start_date'])) ? $arguments['subscription_start_date'] : null;
        $this->subscription_status = (!empty($arguments['subscription_status'])) ? $arguments['subscription_status'] : 'trial';
        $plan = $arguments['plan'] ?? 'monthly';
        switch ($plan) {
            case 'six_months':
                $this->subscription_end_date = date('Y-m-d', strtotime('+6 months', strtotime($this->subscription_start_date)));
                break;
            case 'annual':
                $this->subscription_end_date = date('Y-m-d', strtotime('+1 year', strtotime($this->subscription_start_date)));
                break;
            case 'monthly':
            default:
                $this->subscription_end_date = date('Y-m-d', strtotime('+1 month', strtotime($this->subscription_start_date)));
                break;
        }
        $this->setActionsPermissions();
    }
    ...
    public function get($id)
    {
        ...
        while ($row = $statement->fetch()) {
            ...
            $this->account_request = html_output($row['account_request']);
            $this->trial_start_date = html_output($row['trial_start_date']);
            $this->stripeToken = html_output($row['stripeToken']);
            $this->subscription_start_date = html_output($row['subscription_start_date']);
            $this->subscription_status = html_output($row['subscription_status']);
            $this->subscription_end_date = html_output($row['subscription_end_date']);
            $this->plan = html_output($row['plan']);
    ...
    public function create()
    {
        ...
        $this->stripe_subscription_id = $subscription->id;
            ...
            $statement = $this->dbh->prepare(
                ...
                plan , trial_start_date, stripeToken, subscription_start_date, subscription_end_date, subscription_status, stripe_customer_id, stripe_subscription_id
            )
            VALUES (
            ...
            $statement->bindParam(':can_upload_public', $this->can_upload_public, PDO::PARAM_INT);
                    // Bind new card details fields
            $statement->bindParam(':plan', $this->plan);
            $statement->bindParam(':trial_start_date', $this->trial_start_date);
            $statement->bindParam(':stripeToken', $this->stripeToken);
            $statement->bindParam(':subscription_start_date', $this->subscription_start_date);
            $statement->bindParam(':subscription_end_date', $this->subscription_end_date);
            $statement->bindParam(':subscription_status', $this->subscription_status);
            $statement->bindParam(':stripe_customer_id', $this->stripe_customer_id);
            $statement->bindParam(':stripe_subscription_id', $this->stripe_subscription_id);
            $statement->execute();
